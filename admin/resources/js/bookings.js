/**
 * Bookings calendar - loads FullCalendar from npm and initializes
 */
...
function initBookingsCalendar() {
    ...
    const cancelBase = calendarEl.dataset.cancelBase || '';
    ...
    try {
        const calendar = new Calendar(calendarEl, {
            ...
            eventClick: function (info) {
                ...
                const dateStr = start ? start.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) : '';
                ...
                if (content) {
                    content.innerHTML = items.filter(i => i.value).map(function (i) {
                        let val = escapeHtml(String(i.value));
                ...
                const qrContainer = document.getElementById('booking-modal-qr');
                const qrCanvas = document.getElementById('booking-qr-canvas');
                if (qrCanvas) qrCanvas.innerHTML = '';
                if (qrContainer) {
                    if (p.phone && typeof QRCode !== 'undefined') {
                        const digits = String(p.phone).replace(/\D/g, '');
                        let tel = digits.charAt(0) === '0' ? 'tel:+44' + digits.slice(1) : (digits.length >= 10 ? 'tel:+44' + digits.replace(/^44/, '') : 'tel:' + digits);
        ...
        calendarEl.innerHTML = '<p class="text-red-500 p-4">Calendar failed to load: ' + escapeHtml(String(e.message)) + '</p>';
    }
}

window.printInvoice = function () {
    const id = window._currentBookingId;
    const base = window._bookingInvoiceBase || '';
    if (!id || !base) return;
